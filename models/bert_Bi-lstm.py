...
class Config(object):
    ...
    def __init__(self, dataset):
        ...
        self.vocal_train = dataset + '/data/vocal_train.pth'  # 训练集
        self.vocal_test = dataset + '/data/vocal_test.pth'                                # 测试集
...
class TwoLayerLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_classes, device, num_layers=5, batch_first=True, dropout=0.05):
        ...
        # 定义全连接层用于输出分类结果（这里假设输出类别为num_class）
        self.fc = nn.Linear(hidden_size*2, num_classes)

    def forward(self, x, h0=None, c0=None):
        if h0 is None or c0 is None:
            # LSTM前向传播
            h0 = torch.zeros(self.num_layers*2, x.size(0), self.hidden_size).to(self.device)  # 初始化隐藏状态
            c0 = torch.zeros(self.num_layers*2, x.size(0), self.hidden_size).to(self.device)  # 初始化单元状态
